...
void BoxBlur_float(UnityTexture2D tex,UnitySamplerState ss,float2 uv,float2 offset,float2 texelSize,out float4 resul)
{
    ...
    resul1 =SAMPLE_TEXTURE2D(tex,ss,uv);
    resul1+=SAMPLE_TEXTURE2D(tex,ss,uv+offset*float2(-1,1))*-1;
    resul1+=SAMPLE_TEXTURE2D(tex,ss,uv+offset*float2(0,1))*-2;
    resul1+=SAMPLE_TEXTURE2D(tex,ss,uv+offset*float2(1,1))*-1;
    resul1+=SAMPLE_TEXTURE2D(tex,ss,uv+offset*float2(-1,0))*0;
    resul1+=SAMPLE_TEXTURE2D(tex,ss,uv+offset*float2(1,0))*0;
    resul1+=SAMPLE_TEXTURE2D(tex,ss,uv+offset*float2(-1,-1))*1;
    resul1+=SAMPLE_TEXTURE2D(tex,ss,uv+offset*float2(0,-1))*2;
    resul1+=SAMPLE_TEXTURE2D(tex,ss,uv+offset*float2(1,-1))*1;
    resul=(((resul*resul)+(resul1*resul1))*1/2);
}
#endif 